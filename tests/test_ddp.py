...
def test_ddp_messages():
    ...
    msg = msg.split("\n")
    for item in msg:
        assert item in MOCK_DDP_MESSAGE.format(ddp.DDP_TYPE_SEARCH, MOCK_DDP_VER)
...
def test_get_status():
    ...
    with patch(
        ...
        return_value=(MOCK_DDP_RESPONSE.encode("utf-8"), (MOCK_HOST, MOCK_RANDOM_PORT)),
    ) as mock_send:
...
def test_ddp_disable_polls():
    ...
    # Diabled polls
    mock_ddp._handle(
        MOCK_DDP_RESPONSE_STANDBY.encode(), (mock_ps4.host, MOCK_RANDOM_PORT)
    )
    mock_ddp.send_msg(mock_ps4)
    assert len(mock_ddp._transport.sendto.mock_calls) == 1
...
def test_get_socket_error():
    ...
    with patch("pyps4_2ndscreen.ddp.socket.socket.bind", side_effect=socket.error):
        sock = ddp.get_socket()
        assert sock is None
...
async def start_mock_instance():
    ...
    mock_client_protocol._set_write_port(
        MOCK_DDP_PROTO_PORT
...
def _add_callbacks(client_protocol, ps4_obj, callbacks):
    client_protocol.add_callback(ps4_obj[0], callbacks[0])
    client_protocol.add_callback(ps4_obj[1], callbacks[1])
    client_protocol.add_callback(ps4_obj[2], callbacks[2])
...
async def test_status():
    ...
    assert (
        len(mock_client_protocol.callbacks[MOCK_DDP_PROTO_HOST]) == 2
    )  # noqa: 2 Callbacks for 2 PS4 Object.
    assert (
        len(mock_client_protocol.callbacks[MOCK_DDP_PROTO_HOST2]) == 1
    )  # noqa: 1 Callback for other PS4 device.
