...
def get_ps_store_url(title, region):
...
def get_ps_store_data(title, title_id, region, url=None):
    ...
    try:
        ...
        if not result:
            title = title.split(' ')
            return get_ps_store_data(title[0], title_id, region, url=None)
    ...
    for game_type in type_list:
        for item in result:
            ...
            if _is_game_type(game, game_type):
                ...
                elif title.upper() == _format_title(title_parse):
                    cover_art = _get_cover(game)
                    if cover_art is not None:
                        if has_parent is False:
                            match_title.update({title_parse: cover_art})

    return _get_similar(title, match_id, match_title)
